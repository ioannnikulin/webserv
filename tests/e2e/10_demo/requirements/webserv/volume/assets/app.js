...
const SECTION_STORAGE_KEY = "openSectionId";
let isRestoringSection = false;

const savedSectionId = localStorage.getItem(SECTION_STORAGE_KEY);
if (savedSectionId) {
	const savedSection = document.getElementById(savedSectionId);
	if (savedSection) {
		isRestoringSection = true;
		sections.forEach(section => {
			section.open = section === savedSection;
		});
...
sections.forEach(section => {
	section.addEventListener("toggle", () => {
		if (!section.open) {
			if (localStorage.getItem(SECTION_STORAGE_KEY) === section.id) {
		...
		if (section.id) {
			localStorage.setItem(SECTION_STORAGE_KEY, section.id);
		}
		if (!isRestoringSection) {
